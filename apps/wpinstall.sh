...
function mysql_create_database {
    ...
    echo "CREATE DATABASE $2;" | mysql -h 127.0.0.1 -u root -p$1
}
...
function mysql_create_user {
    ...
    echo "CREATE USER '$2'@'localhost' IDENTIFIED BY '$3';" | mysql -h 127.0.0.1 -u root -p$1
}
...
function mysql_grant_user {
    ...
    echo "GRANT ALL PRIVILEGES ON $3.* TO '$2'@'localhost';" | mysql -h 127.0.0.1 -u root -p$1
    echo "FLUSH PRIVILEGES;" | mysql -h 127.0.0.1 -u root -p$1
...
function wordpress_install {
    ...
    # http://downloads.wordpress.org/plugin/wp-super-cache.0.9.8.zip
        if ! grep -q wp-admin /etc/apache2/apache2.conf; then
        tee --append 1>/dev/null <<WP_CONFIG
...
access_password=`randomString 10` 
htpasswd -b -c  /etc/apache2/.htpasswd admin $access_password

touch /root/wppass
echo "User: admim Pass: $access_password" >> /root/wppass
